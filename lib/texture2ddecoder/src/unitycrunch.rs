...
pub fn decode_unity_crunch(
    ...
    match handler.format {
        CrnFormat::Dxt1 => unsafe { bcn::decode_bc1(&handler.dxt_data, width, height, image) },

        CrnFormat::Etc1 | CrnFormat::Etc1s => unsafe {
            decode_etc1(&handler.dxt_data, width, height, image)
        },
        ...
        | CrnFormat::Dxt5XGbr
        | CrnFormat::Dxt5Agbr
        | CrnFormat::Dxt5XGxR => unsafe {
            bcn::decode_bc3(&handler.dxt_data, width, height, image)
        },

        CrnFormat::Dxt5a => unsafe { bcn::decode_bc4(&handler.dxt_data, width, height, image) },

        CrnFormat::DxnXy | CrnFormat::DxnYx => unsafe {
            bcn::decode_bc5(&handler.dxt_data, width, height, image)
        },

        CrnFormat::Etc2 => unsafe { decode_etc2_rgb(&handler.dxt_data, width, height, image) },

        CrnFormat::Etc2a | CrnFormat::Etc2as => unsafe {
