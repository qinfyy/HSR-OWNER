...
fn applicate_color_4bpp(_data: &[u8], info: &mut [PVRTCTexelInfo; 9], buf: &mut [u32; 32]) {
    ...
    (0..4).for_each(|y| {
        (0..4).for_each(|x| {
            let mut ac = 0;
            for weight_y in INTERP_WEIGHT[y].iter() {
                for weight_x in INTERP_WEIGHT[x].iter() {
                    let interp_weight: i32 = weight_x * weight_y;
...
fn applicate_color_2bpp(_data: &[u8], info: &mut [PVRTCTexelInfo; 9], buf: &mut [u32; 32]) {
    ...
    (0..4).for_each(|y| {
        (0..8).for_each(|x| {
            let mut ac = 0;
            for weight_y in INTERP_WEIGHT_Y[y].iter() {
                for weight_x in INTERP_WEIGHT_X[x].iter() {
                    let interp_weight: i32 = weight_x * weight_y;
                    clr_a[i].r += info[ac].a.r as i32 * interp_weight;
                    clr_a[i].g += info[ac].a.g as i32 * interp_weight;
                    clr_a[i].b += info[ac].a.b as i32 * interp_weight;
        ...
        for by in 0..num_blocks_y {
            ...
            for bx in 0..num_blocks_x {
                ...
                for cy in 0..3 {
                    for cx in 0..3 {
                        local_info[c] =
                            texel_info[morton_index(pos_x[cx], pos_y[cy], min_num_blocks)];
                        c += 1;
                    }
                }
