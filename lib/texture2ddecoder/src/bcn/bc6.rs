...
fn f16_to_u8(h: u16) -> u8 {
    let fp32_val = crate::simd::tables::SIMD_TABLES.fp16_to_fp32[h as usize];
    f32_to_u8(fp32_val)
}
...
pub fn decode_bc6_block(data: &[u8], outbuf: &mut [u32], signed: bool) {
    ...
    unsafe {
        decode_bc6_block_avx2(data, outbuf, signed)
    }

    #[cfg(not(target_arch = "x86_64"))]
    decode_bc6_block_scalar_impl(data, outbuf, signed)
}

fn decode_bc6_block_scalar_impl(data: &[u8], outbuf: &mut [u32], signed: bool) {
    ...
    (0..4_usize).for_each(|yy| {
        (0..4_usize).for_each(|xx| {
            ...
            let rr_u8 = f16_to_u8(rr);
            let gg_u8 = f16_to_u8(gg);
            let bb_u8 = f16_to_u8(bb);
            outbuf[idx] = color(rr_u8, gg_u8, bb_u8, 255);
        });
    });
}
...
#[cfg(target_arch = "x86_64")]
#[inline(always)]
unsafe fn decode_bc6_block_avx2(data: &[u8], outbuf: &mut [u32], signed: bool) {
    use core::arch::x86_64::*;

    let mut temp = [0u32; 16];
    decode_bc6_block_scalar_impl(data, &mut temp, signed);

    for row in 0..4 {
        let dst = outbuf.as_mut_ptr().add(row * 4) as *mut __m128i;
        let src = temp.as_ptr().add(row * 4) as *const __m128i;
