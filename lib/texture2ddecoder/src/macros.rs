macro_rules! block_decoder {
    ($name: expr, $block_width: expr, $block_height: expr, $raw_block_size: expr, $block_decode_func: expr) => {
        paste::item! {
            #[doc = "Decodes a " $name " encoded texture into an image (unsafe)"]
            /// # Safety
            pub unsafe fn [<decode_ $name>](
                data: &[u8],
                width: usize,
                height: usize,
                image: &mut [u32],
                ...
                if total_blocks >= 4096 {
                    ...
                    let blocks_per_row = num_blocks_x;
                    let raw_block = $raw_block_size;

                    image
                        .par_chunks_mut(row_stride * BLOCK_HEIGHT)
                        .enumerate()
                        .for_each(|(by, row_slice)| {
                            let mut local_buffer: [u32; BLOCK_SIZE] = [crate::color::color(0,0,0,255); BLOCK_SIZE];
                            let data_row_offset = by * blocks_per_row * raw_block;
                            let row_height = BLOCK_HEIGHT.min(height.saturating_sub(by * BLOCK_HEIGHT));

                            for bx in 0..blocks_per_row {
                                let data_offset = data_row_offset + bx * raw_block;
                                $block_decode_func(&data[data_offset..], &mut local_buffer);

                                let max_x = width.saturating_sub(bx * BLOCK_WIDTH).min(BLOCK_WIDTH);
                                for y in 0..row_height {
                                    let dst_row = y * row_stride;
                                    let src_row = y * BLOCK_WIDTH;
                                    let dst_offset = dst_row + bx * BLOCK_WIDTH;
                                    row_slice[dst_offset..dst_offset + max_x]
                                        .copy_from_slice(&local_buffer[src_row..src_row + max_x]);
                                }
                            }
                        });
                } else {
