...
pub fn process(type_to_item: &TypeToItemMap) -> HashMap<String, String> {
    ...
    let result = il2cpp::get_cached_class("RPG.Client.EquipmentItemData")
        .and_then(|c| RuntimeType::from_class(c).ok())
        .and_then(|rt| {
            let method = rt
                .get_methods_il2cpp()
                .into_iter()
                .find(|m| m.get_name().is_ok_and(|n| n.as_str() == "Sync"))?;
            let proto = method
                .get_parameters()
                .first()
    ...
    if map.is_empty() {
        log::debug!("[Handler NT] Equipment: no NT generate");
    }
    map
}
