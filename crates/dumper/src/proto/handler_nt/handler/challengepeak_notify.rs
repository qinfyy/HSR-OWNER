...
pub fn process(type_to_item: &TypeToItemMap) -> HashMap<String, String> {
    ...
    let result = il2cpp::get_cached_class("RPG.Client.ChallengePeakSettlement")
        .and_then(|c| RuntimeType::from_class(c).ok())
        .and_then(|rt| {
            let method = rt
                .get_methods_il2cpp()
                .into_iter()
                .find(|m| m.get_name().is_ok_and(|n| n.as_str() == "Create"))?;
            let proto = method
                .get_parameters()
                .first()
    ...
    if proto_reg == Register::None || game_reg == Register::None {
        log::debug!("[Handler NT] ChallengePeakSettleScNotify: reg detection failed");
        return map;
    }
    ...
    if map.is_empty() {
        log::debug!("[Handler NT] ChallengePeakSettlementNotify: no NT generate");
    }
    map
}
