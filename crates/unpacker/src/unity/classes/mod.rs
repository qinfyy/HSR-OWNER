...
use crate::{
    archive::Archive,
    unity::{object::ObjectInfo, serialized_file::SerializedFileHeader},
};
...
pub trait SaveableObject {
    fn name(&self) -> &'_ str;

    fn save_path(&self, container_path: Option<&str>, out_dir: &Path) -> PathBuf {
        let out = if let Some(container_path) = container_path {
            let out = out_dir.join(container_path);
            let extension = out.extension();

            let mut out = out.with_file_name(self.name());
            ...
            out_dir.join(format!("_unmapped/{}", self.name()))
        };
...
impl dyn UnityObject {
    ...
    pub fn downcast<T: Any>(self: Box<Self>) -> Result<Box<T>, Box<dyn UnityObject>> {
        if self.as_ref().type_id() == TypeId::of::<T>() {
