...
use crate::diskcache::Addr;
use crate::diskcache::reader::DiskCacheRead;
    ...
    pub fn from_reader<R: Read + Seek>(mut reader: R) -> Result<Self> {
        ...
        match version {
            ...
            BLOCK_VERSION3_0 => {
                return Err(Error::new(
                    ErrorKind::InvalidData,
                    format!("Unimplemented block file version: 0x{BLOCK_VERSION3_0:X}"),
                ));
            }
            _ => {
                return Err(Error::new(
                    ErrorKind::InvalidData,
                    format!("Unsupported block file version: 0x{version:X}"),
                ));
            }
        }
...
impl BlockFile {
    pub fn read_data(&self, addr: Addr) -> Result<&[u8]> {
        ...
        if !addr.is_block_file() {
            return Err(Error::new(ErrorKind::InvalidInput, "Address is not block file"));
        }
        if addr.file_number() != self.header.this_file as u32 {
            return Err(Error::new(
                ErrorKind::InvalidInput,
                format!(
                    "File number mismatch. Expected: {}, Actual: {}",
        ...
        self.data.get(offset..offset + length).ok_or(Error::new(
            ErrorKind::InvalidInput,
            format!("Illegal address: {addr:?}, offset: {offset}..{}", offset + length),
        ))
    }
}
